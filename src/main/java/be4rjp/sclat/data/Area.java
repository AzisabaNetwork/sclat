...
public class Area {
	...
	public void setupAreaTeam() {
		colorTeam0 = match.getTeam0().getTeam().getScoreboard()
				.registerNewTeam("ColorTeam0" + Main.getNotDuplicateNumber());
		colorTeam0.setCanSeeFriendlyInvisibles(false);
		colorTeam0.setColor(match.getTeam0().getTeamColor().getChatColor());

		colorTeam1 = match.getTeam0().getTeam().getScoreboard()
				.registerNewTeam("ColorTeam1" + Main.getNotDuplicateNumber());
	...
	public void setup(Match match) {
		...
		for (int x = this.from.getBlockX(); x <= this.to.getBlockX(); x++) {
			for (int z = this.from.getBlockZ(); z <= this.to.getBlockZ(); z++) {
				...
				if (x == this.from.getBlockX() || x == this.to.getBlockX() || z == this.from.getBlockZ()
						|| z == this.to.getBlockZ()) {
					Shulker sl = (Shulker) this.from.getWorld().spawnEntity(loc.clone().add(0, 0, 0),
							EntityType.SHULKER);
					sl.setAI(false);
					sl.setGravity(false);
	...
	public void start() {
		this.task.runTaskTimer(Main.getPlugin(), 0, 20);
	}
	...
	public List<Shulker> getShulkerBoxes() {
		return this.slist;
	}
