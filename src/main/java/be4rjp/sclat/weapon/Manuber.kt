...
public class Manuber {
				...
				if (data.getWeaponClass().getMainWeapon().isManeuver) {
					...
					if (data.getIsSneaking() && sl_recharge_2 && !data.getIsSliding()
											...
											for (int i2 = 0; i2 < 4; i2++) {
												o_player.spawnParticle(org.bukkit.Particle.BLOCK_DUST,
														location.clone()
																...
																.add(0, 1.1, 0).add(randomVector.getX(),
						...
						if (sl_recharge_2) {
							...
							task0_1.runTaskTimer(VariablesKt.getPlugin(), 0, 1);
						} else {
							task0_0.runTaskTimer(VariablesKt.getPlugin(), 0, 1);
						}
	...
	public static boolean isSafeLocation(Player player, Location location) {
		...
		BoundingBox playerBoundingBox = new BoundingBox(location.getX() - 0.4, location.getY(), location.getZ() - 0.4,
				location.getX() + 0.4, location.getY() + 1.9, location.getZ() + 0.4
		// location.clone().getX() - 0.4, location.clone().getY(),
