...
public class Hound {
			...
			public void run() {
				...
				if (!data.getIsUsingManeuver() && data.getCanShoot()) {
					...
					if ((clickType == ClickType.FIRST_CLICK || clickType == ClickType.RENDA
							|| clickType == ClickType.NAGAOSI) && data.isInMatch() && data.getCanRollerShoot()) {
						Hound.Shoot(p);
	...
	public static void Shoot(Player player) {
		...
		Vector vec = new Vector(pVector.getX(), 0, pVector.getZ()).normalize()
				.multiply(data.getWeaponClass().getMainWeapon().getShootSpeed());
			...
			public void run() {
				try {
					...
					if (i == explodetick || !player.isOnline() || !data.isInMatch()
							|| (data.getIsSneaking() && data.getArmorlist(0) == as1 && !data.getIsSliding())) {
						...
						for (Entity as : player.getWorld().getEntities()) {
							if (as.getLocation().distance(as1l) <= maxDist) {
								if (as instanceof ArmorStand) {
									double damage = exdamage(heightdiff, maxDist - as.getLocation().distance(as1l),
											data.getWeaponClass().getMainWeapon().getBlasterExDamage()
													* Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP));
