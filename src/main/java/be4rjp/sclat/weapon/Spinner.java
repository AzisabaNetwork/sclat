			...
			public void run() {
				...
				if (data.getTick() == 7 && data.isInMatch()) {
					if (p.getExp() > data.getWeaponClass().getMainWeapon().getNeedInk() * charge) {
						...
						if (reach >= 2) {
							SpinnerShootRunnable((int) (reach * data.getWeaponClass().getMainWeapon().getChargeRatio()),
									p);
						} else {
	...
	public static void SpinnerShootRunnable(int charge, Player player) {
		...
		BukkitRunnable task = new BukkitRunnable() {
			...
			public void run() {
				...
				Spinner.Shoot(p,
						(int) (charge / DataMgr.getPlayerData(p).getWeaponClass().getMainWeapon().getChargeRatio()));
				c++;
			}
		};
		task.runTaskTimer(Main.getPlugin(), 2,
