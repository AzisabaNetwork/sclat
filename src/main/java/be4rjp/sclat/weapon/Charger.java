...
public class Charger {
		...
		BukkitRunnable task = new BukkitRunnable() {
			...
			public void run() {
				...
				if (data.getTick() <= 6 && data.isInMatch()) {
					...
					ArrayList<Vector> positions = rayTrace.traverse(
				...
				if (data.getTick() == 7 && data.isInMatch()) {
					...
					if (charge <= min) {
						if (p.getExp() > data.getWeaponClass().getMainWeapon().getNeedInk()
								* Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP) * charge / 2) {
							p.setExp(p.getExp() - (float) ((data.getWeaponClass().getMainWeapon().getNeedInk() / 2)
									* Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP)
									/ Gear.getGearInfluence(player, Gear.Type.MAIN_INK_EFFICIENCY_UP) * charge));
							Charger.Shoot(p,
									(int) ((double) charge
							...
							* Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP)) {
						p.setExp(p.getExp() - (float) (data.getWeaponClass().getMainWeapon().getNeedInk()
								* Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP)
								/ Gear.getGearInfluence(player, Gear.Type.MAIN_INK_EFFICIENCY_UP) * charge));
						Charger.Shoot(p,
								(int) ((double) charge * (double) data.getWeaponClass().getMainWeapon().getChargeRatio()
	...
	public static void Shoot(Player player, int reach, double damage, double decRate) {
		...
		ArrayList<Vector> positions = rayTrace
				.traverse((int) (reach * Gear.getGearInfluence(player, Gear.Type.MAIN_SPEC_UP)), 0.2);

		Location entityLocation = player.getEyeLocation();
		double distance = reach; // レイの長さ
		World world = player.getWorld();
		double raydistance = distance;
		RayTraceResult rayresult = world.rayTraceBlocks(entityLocation, player.getEyeLocation().getDirection(),
				distance);
