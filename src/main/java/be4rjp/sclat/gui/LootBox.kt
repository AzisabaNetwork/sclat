...
object LootBox {
    private const val Firstprize = 3000
    private const val Secondprize = 1000
    private const val Thirdprize = 500
    private const val Fourthprize = 200
    private const val Fifthprize = 50
    fun turnLootBox(player: Player) {
        ...
        val name = ""
        for (ClassName in Sclat.Companion.conf!!.classConfig!!.getConfigurationSection("WeaponClass")!!
            .getKeys(false)) {
            if (getWeaponClass(ClassName)!!.mainWeapon!!.islootbox) {
                ...
                if (nextLootSeed <= LootSeed && LootSeed < lootpro + nextLootSeed) {
                    ...
                    if (!haveWeapon(player, ClassName)) {
                        ...
                        sendMessage(
                            ChatColor.GREEN.toString() + ClassName + "が重複したよ +" + Firstprize + "coin",
                            MessageType.PLAYER,
                            player,
                        )
                        addMoney(player, Firstprize)
                    }
                }
        ...
        if (nextLootSeed < 5 && !isHit && LootSeed < 5) {
            ...
            addMoney(player, Firstprize)
            sendMessage(
                ChatColor.GREEN.toString() + "「1等!」おめでとう! +" + Firstprize + "coin",
                MessageType.PLAYER,
                player,
            )
        } else if (nextLootSeed < 10 && !isHit && LootSeed < 10) {
            isHit = true
            addMoney(player, Secondprize)
            sendMessage(
                ChatColor.GREEN.toString() + "「2等!」ラッキー! +" + Secondprize + "coin",
                MessageType.PLAYER,
                player,
            )
        } else if (nextLootSeed < 30 && !isHit && LootSeed < 30) {
            isHit = true
            addMoney(player, Thirdprize)
            sendMessage(ChatColor.GREEN.toString() + "「3等」 +" + Thirdprize + "coin", MessageType.PLAYER, player)
        } else if (nextLootSeed < 70 && !isHit && LootSeed < 70) {
            isHit = true
            addMoney(player, Fourthprize)
            sendMessage(ChatColor.GREEN.toString() + "「4等」 +" + Fourthprize + "coin", MessageType.PLAYER, player)
        } else {
            isHit = true
            addMoney(player, Fifthprize)
            sendMessage(ChatColor.GREEN.toString() + "「5等」 +" + Fifthprize + "coin", MessageType.PLAYER, player)
        }
        subTicket(player, 10)
    }
        ...
        for (ClassName in Sclat.Companion.conf!!.classConfig!!.getConfigurationSection("WeaponClass")!!
            ...
            if (getWeaponClass(ClassName)!!.mainWeapon!!.islootbox) {
                ...
                if (Lootpro != 0.0) {
                    ...
                    lores.add(
                        "§r§6SubWeapon : " +
                            Sclat.Companion.conf!!.classConfig!!.getString("WeaponClass." + ClassName + ".SubWeaponName"),
                    )
                    lores.add(
                        "§r§6SPWeapon  : " +
                            Sclat.Companion.conf!!.classConfig!!.getString("WeaponClass." + ClassName + ".SPWeaponName"),
                    )
                    lores.add("")
    ...
    fun GiftWeapon(player: Player, Weapon: String?) {
