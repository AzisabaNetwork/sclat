...
public class SclatUtil {
	...
	public static void sendRestartedServerInfo() {
		...
		commands.add("map " + conf.getServers().getString("ServerName") + " "
				+ DataMgr.getMapRandom(MatchMgr.mapcount == 0 ? 0 : MatchMgr.mapcount - 1).getMapName());
		commands.add("stop");
		StatusClient sc = new StatusClient(conf.getConfig().getString("StatusShare.Host"),
				conf.getConfig().getInt("StatusShare.Port"), commands);
		sc.startClient();
	...
	public static void repelBarrier(Location center, double radius, Player shooter) {
		for (Player player : Sclat.getPlugin().getServer().getOnlinePlayers()) {
			...
			if (player.getWorld() != center.getWorld())
				continue;
			if (playerData.getArmor() < 10000.0)
				continue;
			if (player.getGameMode() == GameMode.SPECTATOR)
				continue;
			if (playerData.getTeam() == DataMgr.getPlayerData(shooter).getTeam())
				continue;
