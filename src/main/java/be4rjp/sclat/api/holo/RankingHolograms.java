...
public class RankingHolograms {
	...
	public RankingHolograms(Player player) {
		...
		for (int i = 0; i < 5; i++) {
			EntityArmorStand armorStand = new EntityArmorStand(nmsWorld, location.getX(),
					location.getY() + 2.0 - (0.4 * (double) i), location.getZ());
			...
			public void run() {
				...
				try {
					...
					if (rankingType == RankingType.KILL) {
						...
						you.setCustomName(CraftChatMessage.fromStringOrNull("§aYou ->> §e" + (kill == 0 ? "-" : ranking)
					...
					if (rankingType == RankingType.PAINT) {
						...
						you.setCustomName(CraftChatMessage.fromStringOrNull("§aYou ->> §e"
								+ (paint == 0 ? "-" : ranking) + "位 §f" + mcid + "  §6Paint(s) : §r" + paint));
					}
					list.add(you);
				} catch (Exception e) {
				}
