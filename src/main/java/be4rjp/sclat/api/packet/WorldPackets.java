...
public class WorldPackets {
    public static boolean broadcastBlockChange(Location location, Material material) {
        PacketContainer packet = Packets.createPacket(PacketType.Play.Server.BLOCK_CHANGE);
    ...
    public static boolean sendWorldBorderWarningPacket(Player player) {
        PacketContainer packet = Packets.createPacket(PacketType.Play.Server.WORLD_BORDER);
    ...
    public static boolean sendWorldBorderWarningClearPacket(Player player) {
        PacketContainer packet = Packets.createPacket(PacketType.Play.Server.WORLD_BORDER);
        ...
        packet.getDoubles().write(2, 30_000_000.0); // Old radius
        packet.getDoubles().write(3, 30_000_000.0); // New radius
    ...
    public static boolean setPlayerFOV(Player player, float fov) {
        PacketContainer packet = Packets.createPacket(PacketType.Play.Server.ABILITIES);
        ...
        // 2. Set the fly speed (index 0) to the player's current fly speed to avoid bugs
        ...
        byte flags = 0;
        if (player.getAllowFlight()) flags |= 0x04;
        if (player.isFlying()) flags |= 0x02;
        // (Add more flags if you need to maintain invulnerability or creative mode status)
        ...
        packet.getBooleans().write(3, player.getGameMode() == org.bukkit.GameMode.CREATIVE);
