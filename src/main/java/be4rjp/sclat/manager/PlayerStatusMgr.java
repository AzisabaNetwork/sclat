...
public class PlayerStatusMgr {
    ...
    public static void setDefaultStatus(Player player) {
        ...
        conf.getPlayerStatus().set("Status." + player.getUniqueId() + ".EquiptClass", conf.getConfig().getString("DefaultClass"));
    ...
    public static void sendHologram(Player player) {
        ...
        as.setCustomName(CraftChatMessage.fromStringOrNull("§aMoney : §r" + getMoney(player) + "  §aLv : §r" + getLv(player)));
        ...
        as1.setCustomName(CraftChatMessage.fromStringOrNull("§6Rank : §r" + getRank(player) + "  [ §b" + RankMgr.toABCRank(getRank(player)) + " §r]"));
