	...
	public static void SuperJumpRunnable(Player player, Location toloc) {
		...
		BukkitRunnable task = new BukkitRunnable() {
			...
			public void run() {
				...
				double py = (Math.pow(Math.abs((positions.size() / 2) - i), 2) * -1 * coef)
						+ (Math.pow(positions.size() / 2, 2) * coef);
				double y = py > 100 ? 100 + py / 2.8 : py;
				Location tloc = new Location(p.getWorld(), position.getX(), y + position.getY(), position.getZ());
				Vector pvec = new Vector(tloc.getX() - p.getLocation().getX(), tloc.getY() - p.getLocation().getY(),
						tloc.getZ() - p.getLocation().getZ()).multiply(0.17);
				p.setVelocity(pvec);
